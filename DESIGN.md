...
### Motion
- **Route and entry:** Header, first-viewport blocks, and footer share one root timeline. Viewport-sized blocks materialize with opacity, a 10px rise, and `0.985` scale on an expo-out curve; route wrappers and oversized blocks use opacity only. First load uses a 400ms tween, and article destinations use 240ms tweens with a 40ms stagger after a 90ms in-place opacity exit with live scroll compensation clipped to one viewport. Home, Resume, and Writing destinations keep the wait-mode boundary but bypass wrapper exit and entrance opacity so navigation never crosses a blank frame. Article destinations retain the next-frame defer for layout cost. Same-path hash changes never remount the route boundary. The exiting desktop table of contents becomes inert and `display: none` immediately, then disposes after the entrance settles.
...
- **Restraint:** Repeated motion is limited to transforms and opacity; `y` and `scale` are reserved for elements no taller than approximately one viewport, while larger trees fade only. Entrances are one-shot, coarse, and never per paragraph. `prefers-reduced-motion: reduce` snaps the header layout, removes entrance transforms, keeps simple destinations instant, and uses a 150ms opacity fade for animated route destinations.
