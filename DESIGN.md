...
- **Scroll Reveals:** Prose and headings remain stationary. Initially visible content renders immediately, while occasional figures, lists, tables, and blockquotes reveal once with a 200ms opacity-only ease-out; nested landmarks defer to their outermost reveal owner.
...
- **Restraint:** Repeated motion is limited to transforms and opacity; scale is reserved for small page-entry blocks, the article index remains stationary below its hero, and article prose never moves during scroll. Occasional article landmarks use opacity only. The editorial hero's 4px blur is an explicit exception authorized for its one-shot mount entrance and never applies to article or route-sized trees. `prefers-reduced-motion: reduce` renders the hero and article landmarks immediately, snaps the header layout, removes entrance transforms, and keeps simple destinations instant.
