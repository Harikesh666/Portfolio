...
### Motion
- **Route and entry:** Header, first-viewport blocks, and footer share one root timeline. Page blocks scale from `0.95` to `1` with opacity on a strong ease-out curve; route wrappers and oversized blocks use opacity only. The initial route and every navigated route explicitly run the same hidden-to-visible entry cycle. Page entrances and every one-shot scroll reveal use a 2s transition; bounded content enters from `16px` below while long lists, tables, and code figures fade only. Home, Resume, and Writing keep an instant outgoing exit while their incoming route plays the full entry animation. Article destinations retain the next-frame defer for layout cost. Same-path hash changes never remount the route boundary. The exiting desktop table of contents becomes inert and `display: none` immediately, then disposes after the entrance settles.
...
- **Restraint:** Repeated motion is limited to transforms and opacity; scale is reserved for small page-entry blocks, and Y-axis reveals are limited to bounded content while larger trees use opacity only. Entrances are one-shot and preserve reading order. `prefers-reduced-motion: reduce` snaps the header layout, removes entrance transforms, keeps simple destinations instant, and uses a 150ms opacity fade for animated route destinations.
